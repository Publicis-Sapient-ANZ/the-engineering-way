...
Allow team members to review designs as their work schedule allows. This document outlines an approach using Git. A similar process can be implemented on top of any versioned documentation system (like Confluence). If using a non git based approach, use this document as a guideline/approach to design a process that works for your team and customer.

## Impact

Async Reviews have the following benefits:

- **Higher Participation & Accessibility**. Engineers do not need to be online and available at the same time as others to review.
...
1. Mark the PR as a Draft. Prefix the title with "DRAFT", "WIP", or "work-in-progress".
2. Set the pull request to automatically merge after approvals and checks have passed. This can indicate to the reviewer the design is complete from the designer's perspective.
