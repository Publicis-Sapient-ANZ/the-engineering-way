...
1. Provision and manage the cloud or on-prem environment runtime for your application (network resources, compute infrastructure, storage, services, etc).
1. Install the target application version across your target environments.
...
It's important to establish a common understanding between the Engineering Lead and application stakeholder(s) around the release strategy / design  during the planning phase of a project. This common understanding includes the deployment and maintenance of the application throughout its SDLC.
...
Code changes released into the *test* environment typically targets the main branch (when doing [trunk](https://devblogs.microsoft.com/devops/release-flow-how-we-do-branching-on-the-vsts-team/#why-trunk-based-development)). _This is the recommended default_

Alternatively you may use release branches (when doing [gitflow](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow)). _Only if the customer prefers gitflow_
...
Cloud based database PaaS support data replication to help enable fully synchronized Blue Green database environments.
